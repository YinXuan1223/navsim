...
def plot_bev_visualization(scene_loader, token: str, evaluation_results: pd.DataFrame, 
    ...
    # --- compute ground truth sub‐metrics & total score ---
    # 1) load metric cache for this token (对应 exp/metric_cache_stage_x 目录)
    cache_root = Path(os.getenv("NAVSIM_EXP_ROOT")) / "metric_cache_stage_1"
    metric_cache = MetricCache(cache_root, token)
    # 2) instantiate simulator & scorer & traffic policy (跟评估 pipeline 一致)
    simulator = PDMSimulator()
    scorer = PDMScorer()  # 如果用 Hydra，可替换成 instantiate(...)
    traffic_policy: AbstractTrafficAgentsPolicy = instantiate(...)
    # 3) run pdm_score，拿到 DataFrame of sub‐metrics
    gt_df, _ = pdm_score(
        metric_cache,
        ground_truth_trajectory,
        ground_truth_trajectory.trajectory_sampling,
        simulator,
        scorer,
        traffic_policy,
    )
    gt_metrics = gt_df.iloc[0].to_dict()
    # 4) 计算总分，并归一化
    gt_raw_score = calculate_scores(gt_metrics)
    gt_norm_score = normalize_score(np.array([gt_raw_score]))[0]
    # 5) 格式化成多行文本
    gt_text = "\n".join([f"{k}: {v:.4f}" for k, v in gt_metrics.items()])
    gt_text += f"\nTotal (norm): {gt_norm_score:.4f}"
    ...
    for i, token in enumerate(stage_one_worse_case_tokens):
        try:
            ...
            plot_bev_visualization(
                ...
                save_path='new_output_ground_truth/stage_one_worse_case',
    ...
    for i, token in enumerate(stage_two_worse_case_tokens):
        try:
            ...
            plot_bev_visualization(
                ...
                sorted_weights=sorted_weights,
                predict_trajectory=gtrs_dense_model_predictions[token]['trajectory'],
                save_path='new_output_ground_truth/stage_two_worse_case',
